 ...
 *
 */
abstract class BaseInstrumentedRDDFunctions(clock: Clock = new Clock()) extends Serializable {
   ...
   */
  protected def recordOperation[A](operation: => A): A = {
   ...
   * See the class-level documentation for a usage example.
   */
  protected def metricsRecorder(): Option[MetricsRecorder] = {
   ...
   */
  protected def recordFunction[B](function: => B, recorder: Option[MetricsRecorder], functionTimer: Timer): B = {
   ...
   */
  protected def instrumentSaveAsNewAPIHadoopFile[K, V](rdd: RDD[(K, V)], path: String,
